...
@export var node_types = ['Sprite2D', 'Sprite3D'] 
...
func _ready() -> void:
	...
	popup.connect('id_pressed', _on_PopupMenu_item_selected)#, [button.selected])
	
	text = NO_NODE_FOUND
	owner.connect("parameters_reset", _on_parameters_reset)
...
func _on_Button_pressed() -> void:
	...
	for i in edited_scene_child.size():
		if (edited_scene_child[i].get_class() in node_types):
...
func _on_PopupMenu_item_selected(id :int):
	...
	#icon = owner.pluginInstance.get_editor_interface().get_inspector().get_icon(node_types[0], "EditorIcons")
	owner.set(owner_reference, editedSceneRoot.get_node(item_name))
	print(editedSceneRoot.get_node(item_name))
